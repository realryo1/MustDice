...
## 概要
FontRenderer は単一行テキスト描画用の基本クラスです（`framework/font.h`）。
UTF-8 文字列を描画し、色変更・アライメント・事前グリフキャッシュに対応します。

## 主な API
- コンストラクタ
  - `FontRenderer(XMFLOAT2 pos, float fontSize, float rotation, XMFLOAT4 color, const std::string& text, TextAlignment align = TA_MIDDLE)`
- 描画
  - `Draw()`
- テキスト変更
